...
#main
getwd()
setwd("C:/Users/manul/Desktop/3rd/Sem2/Statistical Learning/Project1")
...
ggplot(trainData, aes(x=GroupN, y=EDUC, fill=GroupN)) +
  geom_boxplot() +
...
ggplot(trainData, aes(x=as.factor(SES), y=EDUC, fill=as.factor(SES))) +
  geom_boxplot() +
...
ggplot(trainData, aes(x=SES, y=EDUC)) +
  geom_point() +
...
#
ggplot(trainData, aes(x=GroupN, y=nWBV, fill = GroupN)) +
  geom_boxplot() +
  #geom_point(stat = "summary", fun = "mean", col = "red", size = 5) +
  #geom_jitter(color="black", size=0.4, alpha=0.9) +
  scale_fill_brewer(palette = "Set2") +
...
#fig corr plots
df.quant = subset(trainData, select = c(Visit, MR.Delay, Age, EDUC, eTIV, nWBV, ASF))
